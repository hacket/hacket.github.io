...
date_updated: Tuesday, November 11th 2025, 8:42:28 am
title: Magisk模块
author: hacket
categories:
  - Android进阶
...
### MM (Magisk Manager for Recovery Mode)
...
下载 Shamiko: <https://magiskcn.com/shamiko-install.html>
...
1. 安装模块：Shamiko
2. 下载 MT 管理器，授权 ROOT 权限
3. **MT 管理器** 打开目录 `/data/adb/shamiko` 创建 `whitelist` 文件（是文件，不是文件夹。不要搞错了哦）
![image.png](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/202511110819356.png)
4. 重启手机，显示 `whitelist mode` 就是切换成 白名单模式
![image.png](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/202511110819413.png)
**白名单比黑名单有什么优势？**
- 全局隐藏，更彻底，也不用设置排除列表
- 所有 app 都检测不到 root，已授权 root 的 app 可以正常使用 root 权限。
- 需要授权 root，关闭 Shamiko 模块重启手机，授权完再启用 shamiko 模块重启手机即可。

**参考：** [Shamiko 白名单模式（Shamiko whitelist）-](https://magiskcn.com/shamiko-whitelist.html)
...
### 部分软件还能检测 Root，用 LSPosed：`隐藏应用列表`

隐藏 Magisk（面具）后，此时大部分的软件都已经检测不到 root，但是还有一部分顽强的软件还是可以检测到 root 十分顽强：
...
下载 Zygisk 版本的 LSPosed，用面具刷入，重启手机<br>![aw55y](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/aw55y.webp)
...
然后点击 `模板管理`，创建 `黑名单模板`，填写以下可能被检测到的 maps 规则
