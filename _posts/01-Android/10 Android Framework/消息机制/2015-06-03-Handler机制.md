...
date_updated: Thursday, January 30th 2025, 9:50:38 am
title: Handler机制
author: hacket
categories:
  - Android Framework
...
![qply4](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/qply4.webp)

#### Looper

> 用于轮询消息队列中的消息

普通的线程，可执行代码执行完成后，线程生命周期就终止了，线程就会退出；App 主线程如果代码执行完了就自动退出，这是很不合理的。为了防止代码执行完，在代码中插入死循环，代码就不会执行完，线程也就不会退出了。<br />

一个线程进行 Looper.prepare() 和 Looper.loop() 就变成了 Looper 线程（无限循环不退出的线程）<br />
...
消息队列，用来保存消息和安排每个消息的处理顺序。每一个 Looper 线程都有且仅有一个 MessageQueue。<br />
...
![echwd](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/echwd.webp)

#### 消息机制初始化过程

消息机制初始化流程就是 Handler、Looper 和 MessageQueue 三者的初始化流程。<br />
...
![lx2ge](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/lx2ge.webp)
...
![6kjgm](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/6kjgm.png)
...
![xq47d](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/xq47d.png)
