...
比如下载单个文件的时候，secondaryProgress可以表示包含缓冲区的进度，progress表示实际已经写入磁盘的进度

下载多个文件时，secondaryProgress可以表示当前文件下载进度，progress 表示所有文件下载进度

自定义样式的seekbar中，progress和secondaryprogress要根据其所在layerlist中的先后顺序，设置其中后面出现的透明度为非100%不透明的，这样才能让底下的另一个可见
...
这是indeterminate模式下的样式

# ProgressBar问题

## 当前setProgress和上次一样，进度样式未更新
...
就是自定义ProgressBar的`indeterminateDrawable`属性
...
系统默认的ProgressBar样式：`\sdk\platforms\android-17\data\res\values\styles.xml`<br />1、默认的ProgressBar样式
...
`progress_horizontal`为一个xml资源：为一个layer-list文件

### 自定义样式：二级样式

在我们的应用中的styles.xml中新建样式，复写要改变的样式，其他的继承父样式:
...
在drawable目录新建我们的资源文件progress_horizontal.xml
...
为什么是直角的？原因就是被 clip 给切了，所以我们不能够用 clip，而要使用 scale 这个标签。而上面链接给出的解答是定义一个.9的图片就能满足要求，由于我们这里是纯色的一个进度，所以没有必要通过再制作一个.9的图片，而只需要通过同样的方法引用我们定义的一个 shape 就可以了
...
- 进度条的背景设置：`drawable/shape_progressbar_bg.xml` **android:drawable 也可以使用.9图片**
...
在res下创建drawable文件夹，新建文件`drawable/progressbar_color.xml`
