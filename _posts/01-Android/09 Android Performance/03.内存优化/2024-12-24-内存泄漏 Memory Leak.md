...
> getfield 获取指定类(操作符栈顶InnerClassOutClass?)的成员变量(age)，并将其值压入栈顶；即获取InnerClassOutClass的成员变量age的值并放到操作数栈顶
...
非静态内部类访问外部类方法，等价在InnerClassOutClass类新增了access$000方法，每个成员变量生成一个方法：
