...
- if API level < 23, Work Manager checks the availability of Google Play Services _(because some Chinese devices cannot access Google Play Services_😬_)_ if NO, it executes Custom Class that is a mix of `Alarm Manager` and `BroadcastReceiver`
...
- `createSchedulers()` 来根据 Build Version 创建不同的 `Schedulers` 进行任务调度
...
### **SystemJobScheduler (Build Version 大于等于23)**
...
### SystemAlarmScheduler (Build Version 小于23)
