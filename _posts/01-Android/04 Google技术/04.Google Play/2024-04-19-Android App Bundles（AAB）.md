...
## Code transparency for app bundles AAB的代码透明度
...
将APK 上传到 GooglePlay 后，GooglePlay 会替换你的签名，实际上用户下载的 APK 的签名并不是你最初上传的签名，而是 GooglePlay 自己的签名，这个签名存放在 Google 服务器，我们是不能下载的。

**谷歌签名保护计划 ：简单来说就是 开发者上传的应用 会被Google重新进行一次签名 采用Google的签名。**
