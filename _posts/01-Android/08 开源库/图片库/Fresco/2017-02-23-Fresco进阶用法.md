...
date_updated: Saturday, November 1st 2025, 11:44:07 pm
title: Fresco进阶用法
author: hacket
categories:
  - Android进阶
...
![om6v2](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/om6v2.webp)
...
![83on8](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/83on8.webp)
...
使用 SimpleDraweeView 的 FOCUS_CROP 可以实现精确控制图片的裁剪焦点，适用于需要展示图片特定区域的场景（如横幅、背景图等）。

  核心概念

  - **ScaleType.FOCUS_CROP**
	  - 作用：等比例缩放图片填充整个容器，超出部分裁剪
	  - 特点：可通过 actualImageFocusPoint 控制保留图片的哪个部分
  - **actualImageFocusPoint**
  - 类型：PointF
  - 坐标系统：归一化坐标（0.0 到 1.0）
	- (0f, 0f) = 左上角
	- (1f, 1f) = 右下角
	- (0.5f, 0.5f) = 中心点
...
- **左上角** `PointF(0f, 0f)` - 保留图片左上部分，适用于 Logo、标题在左上
- **右上角** `PointF(1f, 0f)` - 保留图片右上部分，适用于关闭按钮、操作区在右上
- **左下角** `PointF(0f, 1f)` - 保留图片左下部分，适用于底部文字、按钮
- **右下角** `PointF(1f, 1f)` - 保留图片右下部分，适用于尾部内容、签名
- **中心** `PointF(0.5f, 0.5f)` - 保留图片中心部分，适用于人物头像、主要内容居中
- **顶部居中** `PointF(0.5f, 0f)` - 保留图片顶部中间，适用于顶部横幅
- **底部居中** `PointF(0.5f, 1f)` - 保留图片底部中间，适用于底部说明文字
