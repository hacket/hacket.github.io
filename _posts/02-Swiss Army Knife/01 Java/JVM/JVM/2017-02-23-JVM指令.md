...
date_updated: Thursday, January 30th 2025, 6:41:57 pm
title: JVM指令
author: hacket
categories:
  - Java&Kotlin
...
一个线程中的方法调用链可能会很长，很多方法都同时处于执行状态。<br>对于 JVM 的执行引擎来 说，在活动线程中，只有位于栈顶的栈帧才是有效的，称为**当前栈帧（Current Stack Frame）**，与这个栈帧相关联的方法称为当前方法（Current Method）。执行引擎运行的所有 字节码指令都只针对当前栈帧进行操作。栈帧的结构 如下图所示：<br>![sxab5](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/sxab5.png)
...
每条指令执行前后局部变量区和操作数栈的变化情况：<br>![plt2x](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/plt2x.png)
...
a = b + c 的字节码执行过程中操作数栈以及局部变量表的变化如下图所示<br>![okthm](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/okthm.png)<br>

![2nhz6](https://raw.githubusercontent.com/hacket/ObsidianOSS/master/obsidian/2nhz6.png)
...
注意处理 Lambda 运行时生成的中间类，

具体见： [[Java8|Java Lambda]]
